...
複数の画像をざっくりとした設定だけでグリッド状に並べて、プレビュー画面で簡単に並び替えができる画像結合Webアプリケーションです。  
...
- 設定するのは結合後の画像の幅×列数のみ。逆算して、各画像のサイズと行数を自動調整します。（高さ×行数も可能）
- プレビュー画面で簡単に並び替え可能。画像間のマージンや切り抜き位置も調整できます。
...
- **グリッド設定**:
    ...
    - **Appearance**: 背景色、画像間のマージン（Gap）調整。
...
- **ズーム機能**: 画面に合わせてプレビューを拡大縮小（Fit Screenでウィンドウサイズに調整）。
